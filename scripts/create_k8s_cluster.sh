...
if [ -z "${SCW_KUBE_PROJECT_NAME}" ];then
    export SCW_KUBE_PROJECT_NAME=$(echo ${APP_GROUP} | tr '[A-Z]' '[a-z]');
fi;

#install bins if needed
${TOOLS_PATH}/scripts/check_install.sh
...
: ${SCW_KUBE_NODES:=1}
: ${SCW_KUBE_MAX_NODES:=10}
: ${SCW_KUBE_VERSION:="1.23.9"}
...
export SCW_KUBE_CLUSTERCONFIG="{'project_id':'${SCW_KUBE_PROJECT_ID}', 'name':'${SCW_KUBE_PROJECT_NAME}-${SCW_ZONE}',
                        ...
                        'pools':[{'name':'default','node_type':'${SCW_FLAVOR}',
                                'autoscaling':true,'size':${SCW_KUBE_NODES},'max_size':${SCW_KUBE_MAX_NODES},'autohealing':true,'zone':'${SCW_ZONE}'}]
                        }"
